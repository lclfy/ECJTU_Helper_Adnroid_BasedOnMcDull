...
public class SelectedCourseIDAdapter extends BaseAdapter {

    private Context context;
    private List<Map<String,String>> list;

    public SelectedCourseIDAdapter(Context context, List<Map<String,String>> list) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        MainImage.setBackgroundResource(R.drawable.ic_img_selectedcourse_id);

        Map<String,String> item = list.get(position);
        tv_FirstLine.setText(item.get("kcmc"));
        String xbmc = "";
        if (item.get("xbmc").contains("【"))
        {
            tv_SecondLine.setText("【"+item.get("xbmc").split("【")[1]);
        }else{
            tv_SecondLine.setText(item.get("xbmc"));
        }
        tv_Right.setText("编号:"+item.get("xbxh"));
